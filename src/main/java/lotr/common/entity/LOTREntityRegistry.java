		...
		try {
			...
			if (!config.exists()) {
				...
				while ((s = bufferedreader.readLine()) != null) {
					...
					if (!s.startsWith("name=") || (s = s.substring("name=".length())).toLowerCase().startsWith("lotr".toLowerCase()) || (i = s.indexOf(",faction=")) < 0 || (j = s.indexOf(",targetEnemies=")) < 0 || (k = s.indexOf(",bonus=")) < 0 || (name = s.substring(0, i)).length() == 0) {
						continue;
					}
