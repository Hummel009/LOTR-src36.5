...
public class LOTREntityInvasionSpawner extends Entity {
	...
	public boolean canInvasionSpawnHere() {
		if (LOTRBannerProtection.isProtected(worldObj, this, LOTRBannerProtection.forInvasionSpawner(this), false)) {
			return false;
		}
		if (LOTREntityNPCRespawner.isSpawnBlocked(this, getInvasionType().invasionFaction)) {
			return false;
		}
	...
	private void announceInvasionTo(EntityPlayer entityplayer) {
		entityplayer.addChatMessage(new ChatComponentTranslation("chat.lotr.invasion.start", new Object[] { getInvasionType().invasionName() }));
	}
	...
	public void selectAppropriateBonusFactions() {
		if (LOTRFaction.controlZonesEnabled(worldObj)) {
			...
			if (bonusFactions.isEmpty()) {
				...
				for (LOTRFaction faction : invasionFaction.getBonusesForKilling()) {
					double dist;
					if (faction.isolationist || ((dist = faction.distanceToNearestControlZoneInRange(posX, posY, posZ, nearestRange)) < 0.0) || nearest != null && (dist >= nearestDist)) {
						continue;
					}
					nearest = faction;
					nearestDist = dist;
