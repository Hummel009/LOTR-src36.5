	...
	public void onBlockInteract(PlayerInteractEvent event) {
		...
		if (event.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK) {
			...
			if (!mightBeAbleToAlterWorld) {
				...
				} else if (block instanceof LOTRBlockArmorStand || block instanceof BlockEnderChest || block instanceof LOTRBlockWeaponRack || block == Blocks.bookshelf) {
				...
				if (!creditHiredUnit) {
					...
					if (!wasSelfDefenceAgainstAlliedUnit && !entityplayer.capabilities.isCreativeMode && entityFaction != LOTRFaction.UNALIGNED) {
						...
						for (Object nearbyAlliedNPC : nearbyAlliedNPCs) {
							...
							if (npc instanceof LOTREntityNPC && ((LOTREntityNPC) npc).hiredNPCInfo.isActive && newAlignment > 0.0f || npc.getAttackTarget() != null) {
								continue;
							}
