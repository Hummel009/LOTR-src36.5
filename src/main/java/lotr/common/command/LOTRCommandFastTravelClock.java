...
public class LOTRCommandFastTravelClock extends CommandBase {
	...
	public void processCommand(ICommandSender sender, String[] args) {
		if (args.length >= 1) {
			...
			int seconds = argSeconds.equals("max") ? 1000000 : CommandBase.parseIntWithMin(sender, args[0], 0);
			if (args.length >= 2) {
			...
			LOTRLevelData.getData(entityplayer).setTimeSinceFT(ticks);
			CommandBase.func_152373_a(sender, (ICommand) this, "commands.lotr.fastTravelClock.set", entityplayer.getCommandSenderName(), seconds, LOTRLevelData.getHMSTime_Seconds(seconds));
			return;
		}
