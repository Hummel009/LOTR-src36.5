	...
	public void processCommand(ICommandSender sender, String[] args) {
		if (args.length >= 2) {
			...
			if (option.equals("add") && args.length >= 3) {
				...
				if (ench != null) {
					if (!LOTREnchantmentHelper.hasEnchant(itemstack, ench) && ench.canApply(itemstack, false) && LOTREnchantmentHelper.checkEnchantCompatible(itemstack, ench)) {
						...
						CommandBase.func_152373_a(sender, (ICommand) this, "commands.lotr.lotrEnchant.add", enchName, entityplayer.getCommandSenderName(), itemstack.getDisplayName());
						return;
					}
			...
			if (option.equals("remove") && args.length >= 3) {
				...
				if (ench != null) {
					if (LOTREnchantmentHelper.hasEnchant(itemstack, ench)) {
						...
						CommandBase.func_152373_a(sender, (ICommand) this, "commands.lotr.lotrEnchant.remove", enchName, entityplayer.getCommandSenderName(), itemstack.getDisplayName());
						return;
					}
			...
			if (option.equals("clear")) {
				...
				CommandBase.func_152373_a(sender, (ICommand) this, "commands.lotr.lotrEnchant.clear", entityplayer.getCommandSenderName(), itemstack.getDisplayName());
				return;
			}
		}
