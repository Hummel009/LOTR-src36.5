...
public class LOTRWorldGenBlueMountainsStronghold extends LOTRWorldGenStructureBase {
	...
	private void generateFacingSouth(World world, Random random, int i, int j, int k) {
		...
		for (i12 = i - 6; i12 <= i + 6; ++i12) {
			...
			for (j1 = j; !LOTRMod.isOpaque(world, i12, j1, k - 7) && j1 >= 0; --j1) {
				setBlockAndNotifyAdequately(world, i12, j1, k - 7, LOTRMod.pillar, 3);
				setGrassToDirt(world, i12, j1 - 1, k - 7);
			}
		}
		for (j12 = j + 1; j12 <= j + 2; ++j12) {
			setAir(world, i, j12, k - 6);
			setBlockAndNotifyAdequately(world, i - 1, j12, k - 7, LOTRMod.pillar, 3);
			setBlockAndNotifyAdequately(world, i + 1, j12, k - 7, LOTRMod.pillar, 3);
		}
		...
		for (j12 = j + 1; j12 <= j + 3; ++j12) {
			for (k1 = k - 4; k1 <= k - 1; ++k1) {
				for (i1 = i - 5; i1 <= i - 1; ++i1) {
					setBlockAndNotifyAdequately(world, i1, j12, k1, LOTRMod.brick, 14);
				}
			...
			for (k1 = k - 2; k1 <= k + 5; ++k1) {
				setBlockAndNotifyAdequately(world, i - 1, j12, k1, LOTRMod.brick, 14);
				setBlockAndNotifyAdequately(world, i + 1, j12, k1, LOTRMod.brick, 14);
			}
		}
	...
	private void generateFacingWest(World world, Random random, int i, int j, int k) {
		...
		for (k12 = k - 6; k12 <= k + 6; ++k12) {
			setBlockAndNotifyAdequately(world, i + 7, j + 1, k12, LOTRMod.slabSingle3, 0);
			setGrassToDirt(world, i + 7, j, k12);
			for (j1 = j; !LOTRMod.isOpaque(world, i + 7, j1, k12) && j1 >= 0; --j1) {
				setBlockAndNotifyAdequately(world, i + 7, j1, k12, LOTRMod.pillar, 3);
				setGrassToDirt(world, i + 7, j1 - 1, k12);
			}
		}
		for (j12 = j + 1; j12 <= j + 2; ++j12) {
			setAir(world, i + 6, j12, k);
			setBlockAndNotifyAdequately(world, i + 7, j12, k - 1, LOTRMod.pillar, 3);
			setBlockAndNotifyAdequately(world, i + 7, j12, k + 1, LOTRMod.pillar, 3);
		}
		...
		for (j12 = j + 1; j12 <= j + 3; ++j12) {
			for (i1 = i + 4; i1 >= i + 1; --i1) {
				for (k13 = k - 5; k13 <= k - 1; ++k13) {
					setBlockAndNotifyAdequately(world, i1, j12, k13, LOTRMod.brick, 14);
				}
			...
			for (i1 = i + 2; i1 >= i - 5; --i1) {
				setBlockAndNotifyAdequately(world, i1, j12, k - 1, LOTRMod.brick, 14);
				setBlockAndNotifyAdequately(world, i1, j12, k + 1, LOTRMod.brick, 14);
			}
		}
