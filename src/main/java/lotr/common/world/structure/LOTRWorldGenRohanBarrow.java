...
public class LOTRWorldGenRohanBarrow extends LOTRWorldGenStructureBase {
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		for (i1 = i - radius; i1 <= i + radius; ++i1) {
			for (int j13 = j + height; j13 >= j; --j13) {
				for (k12 = k - radius; k12 <= k + radius; ++k12) {
					...
					setBlockAndNotifyAdequately(world, i1, j13, k12, grass ? Blocks.grass : Blocks.dirt, 0);
		...
		for (int l = 0; l < 12; ++l) {
			...
			if (world.getBlock(i12, (j14 = world.getHeightValue(i12, k12 = k - random.nextInt(radius) + random.nextInt(radius))) - 1, k12) != Blocks.grass) {
				continue;
			}
			setBlockAndNotifyAdequately(world, i12, j14, k12, LOTRMod.simbelmyne, 0);
		}
		j += height;
		...
		for (int j15 = j - 3; j15 >= j - 4; --j15) {
			for (i12 = i - 4; i12 <= i + 4; ++i12) {
				for (k12 = k - 1; k12 <= k + 1; ++k12) {
					setBlockAndNotifyAdequately(world, i12, j15, k12, Blocks.air, 0);
					if (Math.abs(i12 - i) <= 2) {
						continue;
					}
					setBlockAndNotifyAdequately(world, i12, j - 5, k12, LOTRMod.brick, 4);
				}
			}
