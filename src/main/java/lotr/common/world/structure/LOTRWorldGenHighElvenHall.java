	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		for (i14 = i; i14 <= i + 15; ++i14) {
			for (k12 = k; k12 <= k + 15; ++k12) {
				for (j12 = j; (j12 == j || !LOTRMod.isOpaque(world, i14, j12, k12)) && j12 >= 0; --j12) {
					setBlockAndNotifyAdequately(world, i14, j12, k12, LOTRMod.brick3, 2);
					setGrassToDirt(world, i14, j12 - 1, k12);
				}
				for (j12 = j + 1; j12 <= j + 4; ++j12) {
					setBlockAndNotifyAdequately(world, i14, j12, k12, Blocks.air, 0);
				}
				if (i14 < i + 2 || i14 > i + 13 || k12 < k + 2 || k12 > k + 13) {
					setBlockAndNotifyAdequately(world, i14, j + 5, k12, LOTRMod.brick3, 2);
				} else {
					setBlockAndNotifyAdequately(world, i14, j + 5, k12, plankBlock, plankMeta);
				}
		...
		for (int j14 = j; j14 <= j + 5; j14 += 5) {
			...
			for (k12 = k; k12 <= k + 15; k12 += 15) {
				...
				for (i15 = i; i15 <= i + 15; i15 += 3) {
					for (j2 = j14 + 1; j2 <= j14 + 4; ++j2) {
						setBlockAndNotifyAdequately(world, i15, j2, k12, LOTRMod.pillar, 10);
					}
					setBlockAndNotifyAdequately(world, i15, j14 + 5, k12, LOTRMod.brick3, 2);
				}
				for (i15 = i + 1; i15 <= i + 13; i15 += 3) {
					setBlockAndNotifyAdequately(world, i15, j14 + 5, k12, LOTRMod.stairsHighElvenBrick, 5);
					setBlockAndNotifyAdequately(world, i15 + 1, j14 + 5, k12, LOTRMod.stairsHighElvenBrick, 4);
				}
			}
			...
			for (k12 = k; k12 <= k + 15; k12 += 3) {
				setBlockAndNotifyAdequately(world, i + 1, j14 + 4, k12, LOTRMod.highElvenTorch, 1);
				setBlockAndNotifyAdequately(world, i + 14, j14 + 4, k12, LOTRMod.highElvenTorch, 2);
			}
		}
		int roofWidth = 18;
		int roofX = i - 1;
		...
		while (roofWidth > 2) {
			...
			for (i13 = roofX + 1; i13 < roofX + roofWidth - 2; ++i13) {
				for (k14 = roofZ + 1; k14 < roofZ + roofWidth - 2; ++k14) {
					setBlockAndNotifyAdequately(world, i13, roofY, k14, Blocks.air, 0);
				}
			}
		...
		for (i13 = roofX; i13 < roofX + roofWidth; ++i13) {
			for (k14 = roofZ; k14 < roofZ + roofWidth; ++k14) {
				setBlockAndNotifyAdequately(world, i13, roofY - 1, k14, LOTRMod.glass, 0);
			}
		}
		...
		for (int j16 = j + 3; j16 <= j + 8; j16 += 5) {
			...
			for (k14 = k + 3; k14 <= k + 12; k14 += 3) {
				placeWallBanner(world, i, j16, k14, 3, bannerType);
				placeWallBanner(world, i + 15, j16, k14, 1, bannerType);
			}
		}
