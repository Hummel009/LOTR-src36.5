...
public class LOTRWorldGenElfLordHouse extends LOTRWorldGenStructureBase {
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		if (restrictions) {
			for (i1 = i - 14; i1 <= i + 14; ++i1) {
				for (j1 = j; j1 <= j + 7; ++j1) {
					for (k13 = k - 14; k13 <= k + 14; ++k13) {
						if (Math.abs(i1 - i) <= 2 && Math.abs(k13 - k) <= 2 || world.isAirBlock(i1, j1, k13)) {
							continue;
						}
						return false;
					}
			...
			for (int i14 = i - 2; i14 <= i + 2; ++i14) {
				for (k1 = k - 2; k1 <= k + 2; ++k1) {
					for (j12 = j; !LOTRMod.isOpaque(world, i14, j12, k1) && j12 >= 0; --j12) {
						setBlockAndNotifyAdequately(world, i14, j12, k1, LOTRMod.wood, 1);
					}
				}
			}
			...
			j += MathHelper.floor_double(j1 * MathHelper.randomFloatClamp(random, LOTRWorldGenMallornExtreme.HOUSE_HEIGHT_MIN, LOTRWorldGenMallornExtreme.HOUSE_HEIGHT_MAX));
		}
		buildStaircase(world, random, i, j, k);
		for (i1 = i - 14; i1 <= i + 14; ++i1) {
			for (j1 = j; j1 <= j + 6; ++j1) {
				for (k13 = k - 14; k13 <= k + 14; ++k13) {
					setBlockAndNotifyAdequately(world, i1, j1, k13, Blocks.air, 0);
				}
			}
		}
		...
		for (i1 = i - 4; i1 <= i + 4; i1 += 8) {
			for (k1 = k - 4; k1 <= k + 4; k1 += 8) {
				...
				setBlockAndNotifyAdequately(world, i1 + 1, j + 3, k1, LOTRWorldGenElfHouse.getRandomTorch(random), 1);
				setBlockAndNotifyAdequately(world, i1 - 1, j + 3, k1, LOTRWorldGenElfHouse.getRandomTorch(random), 2);
				setBlockAndNotifyAdequately(world, i1, j + 3, k1 + 1, LOTRWorldGenElfHouse.getRandomTorch(random), 3);
				setBlockAndNotifyAdequately(world, i1, j + 3, k1 - 1, LOTRWorldGenElfHouse.getRandomTorch(random), 4);
			}
		}
		...
		for (i1 = i - 8; i1 <= i + 8; i1 += 16) {
			for (k1 = k - 8; k1 <= k + 8; k1 += 16) {
				for (j12 = j + 2; j12 <= j + 5; ++j12) {
					setBlockAndNotifyAdequately(world, i1, j12, k1, LOTRMod.planks, 1);
				}
			...
			if (l1 < 5) {
				...
				for (i2 = i1; i2 >= i1 - 1; --i2) {
					for (k23 = k1; k23 <= k1 + 1; ++k23) {
						setBlockAndNotifyAdequately(world, i2, j1, k23, LOTRMod.planks, 1);
					}
				}
