...
public class LOTRWorldGenHarnedorMarket extends LOTRWorldGenHarnedorStructure {
	...
	public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
		...
		for (i12 = -1; i12 <= 1; ++i12) {
			int j12;
			for (int step = 0; step < 12 && !isOpaque(world, i12, j12 = 0 - step, k1 = -9 - step); ++step) {
				setBlockAndMetadata(world, i12, j12, k1, plank2StairBlock, 2);
		...
		for (i12 = -1; i12 <= 1; ++i12) {
			int j13;
			for (int step = 0; step < 12 && !isOpaque(world, i12, j13 = 0 - step, k1 = 9 + step); ++step) {
				setBlockAndMetadata(world, i12, j13, k1, plank2StairBlock, 3);
		...
		for (k12 = -1; k12 <= 1; ++k12) {
			int j14;
			for (int step = 0; step < 12 && !isOpaque(world, i1 = -9 - step, j14 = 0 - step, k12); ++step) {
				setBlockAndMetadata(world, i1, j14, k12, plank2StairBlock, 1);
		...
		for (k12 = -1; k12 <= 1; ++k12) {
			int j15;
			for (int step = 0; step < 12 && !isOpaque(world, i1 = 9 + step, j15 = 0 - step, k12); ++step) {
				setBlockAndMetadata(world, i1, j15, k12, plank2StairBlock, 0);
				setGrassToDirt(world, i1, j15 - 1, k12);
				j2 = j15 - 1;
				while (!isOpaque(world, i1, j2, k12) && getY(j2) >= 0) {
					setBlockAndMetadata(world, i1, j2, k12, plank2Block, plank2Meta);
					setGrassToDirt(world, i1, j2 - 1, k12);
