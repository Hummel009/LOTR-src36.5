...
public class LOTRWorldGenNearHaradTent extends LOTRWorldGenHarnedorStructure {
	...
	public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
		...
		for (j12 = 1; j12 <= 5; ++j12) {
			setBlockAndMetadata(world, 0, j12, 0, fenceBlock, fenceMeta);
		}
		...
		for (j12 = 2; j12 <= 3; ++j12) {
			for (int i12 = -1; i12 <= 1; ++i12) {
				setBlockAndMetadata(world, i12, j12, -3, Blocks.wool, 15);
				setBlockAndMetadata(world, i12, j12, 3, Blocks.wool, 15);
			}
			for (k1 = -1; k1 <= 1; ++k1) {
				setBlockAndMetadata(world, -3, j12, k1, Blocks.wool, 15);
				setBlockAndMetadata(world, 3, j12, k1, Blocks.wool, 15);
			}
		}
		setAir(world, 0, 2, -3);
