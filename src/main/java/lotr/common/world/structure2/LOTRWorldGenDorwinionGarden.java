		...
		for (int i1 = -r; i1 <= r; ++i1) {
			for (int k1 = -r; k1 <= r; ++k1) {
				...
				for (j12 = 0; j12 >= -h; --j12) {
					...
					boolean grass = !isOpaque(world, i1, j12 + 1, k1);
					setBlockAndMetadata(world, i1, j12, k1, grass ? Blocks.grass : Blocks.dirt, 0);
				...
				if (i2 == 2 && k2 <= 1 || k2 == 2 && i2 <= 1) {
					for (j1 = -1; j1 <= 1; ++j1) {
						setBlockAndMetadata(world, i1, j1, k1, LOTRMod.brick5, 2);
					}
				}
				if (i2 > 1 || k2 > 1) {
					continue;
