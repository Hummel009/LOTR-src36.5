...
public class LOTRWorldGenTauredainPyramid extends LOTRWorldGenStructureBase2 {
	...
	public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
		...
		for (i12 = -RADIUS; i12 <= RADIUS; ++i12) {
			for (k12 = -RADIUS; k12 <= RADIUS; ++k12) {
				j1 = 0;
				while ((getY(j1) >= originY || !isOpaque(world, i12, j1, k12)) && getY(j1) >= 0) {
		...
		for (i1 = -37; i1 <= 37; ++i1) {
			for (k1 = -37; k1 <= 37; ++k1) {
				...
				for (j112 = roomFloor + 1; j112 < actingRoomTop; ++j112) {
					setAir(world, i1, j112, k1);
				}
				if (i2 > roomPillarEdge || k2 > roomPillarEdge) {
					for (j112 = roomBottom + 1; j112 <= roomFloor + 1; ++j112) {
						placeRandomBrick(world, random, i1, j112, k1);
					}
					continue;
				}
				if (i2 == roomPillarEdge || k2 == roomPillarEdge) {
					for (j112 = roomBottom + 1; j112 <= roomFloor + 1; ++j112) {
						setBlockAndMetadata(world, i1, j112, k1, LOTRMod.brick4, 4);
					}
					placeRandomBrick(world, random, i1, actingRoomTop - 1, k1);
					if (isGolden) {
						setBlockAndMetadata(world, i1, actingRoomTop - 2, k1, LOTRMod.pillar2, 11);
					} else {
						setBlockAndMetadata(world, i1, actingRoomTop - 2, k1, LOTRMod.pillar, 14);
					}
					...
					if (i2 == roomPillarEdge && k3 == 0 || k2 == roomPillarEdge && i3 == 0) {
						for (j111 = roomFloor + 2; j111 <= actingRoomTop - 2; ++j111) {
							if (isGolden) {
								setBlockAndMetadata(world, i1, j111, k1, LOTRMod.pillar2, 11);
								continue;
							}
							setBlockAndMetadata(world, i1, j111, k1, LOTRMod.pillar, 14);
						}
					}
					if (i2 == roomPillarEdge) {
						if (k3 == 1) {
							placeRandomStairs(world, random, i1, actingRoomTop - 3, k1, 7);
							continue;
						}
						if (k3 != 3) {
							continue;
						}
						placeRandomStairs(world, random, i1, actingRoomTop - 3, k1, 6);
						continue;
					}
					...
					if (i3 == 1) {
						placeRandomStairs(world, random, i1, actingRoomTop - 3, k1, 4);
						continue;
					}
					if (i3 != 3) {
						continue;
					}
					placeRandomStairs(world, random, i1, actingRoomTop - 3, k1, 5);
					continue;
				}
				if (i2 <= 10 && k2 <= 10) {
					...
					for (j111 = roomBottom + 1; j111 <= roomFloor; ++j111) {
						placeRandomBrick(world, random, i1, j111, k1);
					}
					int lvlMin = roomFloor + 1;
					int lvlMax = lvlMin + height;
					for (int j113 = lvlMin; j113 <= lvlMax; ++j113) {
						placeRandomBrick(world, random, i1, j113, k1);
					}
					if (max > 3 && max % 2 == 0) {
						setBlockAndMetadata(world, i1, lvlMax, k1, LOTRMod.brick4, 4);
						if (i2 == k2) {
							setBlockAndMetadata(world, i1, lvlMax, k1, LOTRMod.pillar2, 11);
							setBlockAndMetadata(world, i1, lvlMax + 1, k1, LOTRMod.pillar2, 11);
							setBlockAndMetadata(world, i1, lvlMax + 2, k1, LOTRMod.tauredainDoubleTorch, 0);
							setBlockAndMetadata(world, i1, lvlMax + 3, k1, LOTRMod.tauredainDoubleTorch, 1);
						}
					}
					if (max <= 3 || i2 > 1 && k2 > 1) {
						continue;
					...
					if (max % 2 == 0) {
						setBlockAndMetadata(world, i1, lvlMax, k1, LOTRMod.slabSingle8, 3);
						continue;
					}
					setBlockAndMetadata(world, i1, lvlMax, k1, LOTRMod.brick4, 3);
					continue;
				}
				for (j112 = roomBottom + 1; j112 <= roomFloor; ++j112) {
					setBlockAndMetadata(world, i1, j112, k1, Blocks.lava, 0);
				}
				if (random.nextInt(300) == 0) {
					setBlockAndMetadata(world, i1, actingRoomTop, k1, Blocks.flowing_lava, 0);
				}
				if (i2 == roomPillarEdge - 1 || k2 == roomPillarEdge - 1) {
					if (random.nextInt(4) <= 0) {
						continue;
					}
					setBlockAndMetadata(world, i1, roomFloor, k1, Blocks.obsidian, 0);
					continue;
				}
				...
				if (random.nextInt(16) == 0) {
					placeRandomBrick(world, random, i1, roomFloor, k1);
				}
				...
				for (j111 = roomBottom + 1; j111 < actingRoomTop; ++j111) {
					setBlockAndMetadata(world, i1, j111, k1, pillarBlock, pillarMeta);
				}
			}
		}
		...
		for (i1 = -1; i1 <= 1; ++i1) {
			for (k1 = -1; k1 <= 1; ++k1) {
				setAir(world, maze3EndX + i1, stepY, maze3EndZ + k1);
				setAir(world, maze3EndX + i1, stepY - 1, maze3EndZ + k1);
			}
		}
