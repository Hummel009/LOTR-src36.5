...
public abstract class LOTRWorldGenElvenForge extends LOTRWorldGenStructureBase2 {
	...
	public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
		...
		for (i1 = -4; i1 <= 4; ++i1) {
			for (k12 = -4; k12 <= 4; ++k12) {
				layFoundation(world, i1, k12, random);
			}
		}
		for (i1 = -2; i1 <= 2; ++i1) {
		...
		for (int l = 0; l <= 3; ++l) {
			...
			for (k1 = -width + 1; k1 <= width - 1; ++k1) {
				placeRoofStairs(world, -width, j12, k1, 1, random);
				placeRoofStairs(world, width, j12, k1, 0, random);
				placeRoofStairs(world, -width, j12 - 1, k1, 4, random);
				placeRoofStairs(world, width, j12 - 1, k1, 5, random);
			}
			if (l >= 3) {
				continue;
			}
		...
		for (i1 = -5; i1 <= 5; ++i1) {
			for (k12 = -5; k12 <= 5; ++k12) {
				...
				placePillar(world, i1, 1, k12, random);
			}
		}
		if (!ruined) {
			for (i1 = -4; i1 <= 4; i1 += 8) {
		...
		for (i1 = -1; i1 <= 1; ++i1) {
			for (k12 = -1; k12 <= 1; ++k12) {
				...
				k2 = Math.abs(k12);
				if (i2 == 1 && k2 == 1) {
					placePillar(world, i1, 2, k12, random);
					placePillar(world, i1, 3, k12, random);
					placeSlab(world, i1, 4, k12, false, random);
				}
				if (i2 + k2 != 1) {
					continue;
				}
				for (j1 = 2; j1 <= 9; ++j1) {
					placeBrick(world, i1, j1, k12, random);
				}
			}
		}
		if (!ruined) {
