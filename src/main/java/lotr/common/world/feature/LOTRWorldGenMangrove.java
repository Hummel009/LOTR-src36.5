...
public class LOTRWorldGenMangrove extends WorldGenAbstractTree {
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		if (j >= 1 && j + height + 1 <= 256) {
			...
			canGrow = below.canSustainPlant((IBlockAccess) world, i, j - 1, k, ForgeDirection.UP, (IPlantable) Blocks.sapling) || below.canSustainPlant(world, i, j - 1, k, ForgeDirection.UP, Blocks.deadbush);
			if (canGrow) {
				int j1;
