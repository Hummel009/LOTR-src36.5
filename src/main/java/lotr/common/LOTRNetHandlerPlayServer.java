	...
	public void processMountControl(LOTRPacketMountControl packet) {
		...
		if (mount != null && mount != playerEntity && mount.riddenByEntity == playerEntity && LOTRMountFunctions.isMountControllable(mount)) {
			...
			if (distSq - speedSq > 150.0 && (!server.isSinglePlayer() || !server.getServerOwner().equals(playerEntity.getCommandSenderName()))) {
				FMLLog.warning(mount.getCommandSenderName() + " (mount of " + playerEntity.getCommandSenderName() + ") moved too quickly! " + (distSq - speedSq), new Object[0]);
			...
			if (distSq > 10.0) {
				...
				FMLLog.warning(mount.getCommandSenderName() + " (mount of " + playerEntity.getCommandSenderName() + ") moved wrongly! " + dx + ", " + dy + ", " + dz, new Object[0]);
			}
			...
			if (!server.isFlightAllowed() && !world.checkBlockCollision(flyCheckBox)) {
				if (movedY >= -0.03125) {
					...
					if (floatingMountTick > 80) {
						FMLLog.warning(playerEntity.getCommandSenderName() + " was kicked for floating too long on mount " + mount.getCommandSenderName() + "!", new Object[0]);
