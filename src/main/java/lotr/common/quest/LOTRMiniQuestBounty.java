	...
	public void onKill(EntityPlayer entityplayer, EntityLivingBase entity) {
		if (!killed && !isFailed() && entity instanceof EntityPlayer && ((EntityPlayer) entity).getUniqueID().equals(targetID)) {
			...
			if (highestFaction != null) {
				...
				ChatComponentTranslation slainMsg1 = new ChatComponentTranslation("chat.lotr.bountyKilled1", new Object[] { entityplayer.getCommandSenderName(), entityFaction.factionName() });
			...
			ChatComponentTranslation announceMsg = new ChatComponentTranslation("chat.lotr.bountyKill", new Object[] { entityplayer.getCommandSenderName(), slainPlayer.getCommandSenderName(), entityFaction.factionName() });
	...
	public void onKilledByPlayer(EntityPlayer entityplayer, EntityPlayer killer) {
		if (!killed && !isFailed() && killer.getUniqueID().equals(targetID)) {
			...
			if ((curAlignment = (pd = getPlayerData()).getAlignment(entityFaction)) > 100.0f) {
				...
				ChatComponentTranslation slainMsg2 = new ChatComponentTranslation("chat.lotr.killedByBounty2", new Object[] { entityFaction.factionName() });
				slainMsg1.getChatStyle().setColor(EnumChatFormatting.YELLOW);
				slainMsg2.getChatStyle().setColor(EnumChatFormatting.YELLOW);
