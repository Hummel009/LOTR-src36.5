		...
		if (entityplayer.isSneaking() && entityplayer.getHeldItem() == null && npc.getFaction() == pickpocketFaction && npc instanceof IPickpocketable) {
			...
			if (ppable.canPickpocket() && !pickpocketedEntityIDs.contains(id)) {
				...
				if (npc.getAttackTarget() != null) {
					entityplayer.addChatMessage(new ChatComponentTranslation("chat.lotr.pickpocket.inCombat", new Object[0]));
					return true;
				}
				if (isEntityWatching(npc, entityplayer)) {
					entityplayer.addChatMessage(new ChatComponentTranslation("chat.lotr.pickpocket.watched", new Object[0]));
					return true;
				}
				...
				if (success) {
					...
					entityplayer.addChatMessage(new ChatComponentTranslation("chat.lotr.pickpocket.success", new Object[] { picked.stackSize, picked.getDisplayName(), npc.getNPCName() }));
					...
					entityplayer.addChatMessage(new ChatComponentTranslation("chat.lotr.pickpocket.missed", new Object[] { npc.getNPCName() }));
				...
				if (noticed) {
					entityplayer.addChatMessage(new ChatComponentTranslation("chat.lotr.pickpocket.noticed", new Object[] { npc.getNPCName() }));
				...
				if (!noticed || rand.nextFloat() < 0.5f) {
					...
					for (Object o : nearbyFriends) {
						...
						entityplayer.addChatMessage(new ChatComponentTranslation("chat.lotr.pickpocket.otherNoticed", new Object[] { otherNPC.getEntityClassName() }));
