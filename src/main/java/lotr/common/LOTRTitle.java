...
public class LOTRTitle {
	...
	public boolean canPlayerUse(EntityPlayer entityplayer) {
		...
		case ALIGNMENT: {
			for (LOTRFaction f : alignmentFactions) {
				if (LOTRLevelData.getData(entityplayer).getAlignment(f) < alignmentRequired) {
					continue;
				}
				return true;
			}
	...
	public static class PlayerTitle {
		...
		public IChatComponent getFullTitleComponent(EntityPlayer entityplayer) {
			IChatComponent component = new ChatComponentText("[").appendSibling(new ChatComponentTranslation(theTitle.getUntranslatedName(entityplayer), new Object[0])).appendText("]").appendText(" ");
