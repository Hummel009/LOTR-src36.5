...
public class LOTRTickHandlerClient {
	...
	public void onRenderTick(TickEvent.RenderTickEvent event) {
		...
		if (event.phase == TickEvent.Phase.START) {
			...
			if (cancelItemHighlight && LOTRReflectionClient.getHighlightedItemTicks(guiIngame = minecraft.ingameGUI) > 0) {
		...
		if (world != null && entityplayer != null) {
			...
			if (event.type == RenderGameOverlayEvent.ElementType.HELMET) {
				...
				if (playersInElvenPortals.containsKey(entityplayer) && (i = (Integer) playersInElvenPortals.get(entityplayer)) > 0) {
					renderOverlay(null, 0.1f + (float) i / (float) entityplayer.getMaxInPortalTime() * 0.6f, mc, elvenPortalOverlay);
				}
				if (playersInMorgulPortals.containsKey(entityplayer) && (i = (Integer) playersInMorgulPortals.get(entityplayer)) > 0) {
					renderOverlay(null, 0.1f + (float) i / (float) entityplayer.getMaxInPortalTime() * 0.6f, mc, morgulPortalOverlay);
				}
	...
	public void onRenderFog(EntityViewRenderEvent.RenderFogEvent event) {
		...
		if (provider instanceof LOTRWorldProvider) {
			...
			if (LOTRConfig.newWeather && (lotrbiome.getEnableRain() || lotrbiome.getEnableSnow()) && (rain = prevRainFactor + (rainFactor - prevRainFactor) * renderTick) > 0.0f) {
