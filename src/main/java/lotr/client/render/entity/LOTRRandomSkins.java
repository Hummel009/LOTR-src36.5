...
public class LOTRRandomSkins implements IResourceManagerReloadListener {
	...
	private static class LOTRRandomSkinsCombinatorial extends LOTRRandomSkins {
		...
		protected void loadAllRandomSkins() {
			...
			block2: for (String layer : skinLayers) {
				...
				for (ResourceLocation overlay : layerSkins.getAllSkins()) {
					try {
						...
						FMLLog.severe("LOTR: Error combining skins " + skinPath + " on layer " + layer + "!", new Object[0]);
						e.printStackTrace();
						break block2;
					}
				}
			...
			FMLLog.info("LOTR: Assembled combinatorial skins for " + skinPath + ": " + skins.size() + " skins", new Object[0]);
		}
	}
