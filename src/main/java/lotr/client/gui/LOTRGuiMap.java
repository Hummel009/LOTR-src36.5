...
public class LOTRGuiMap extends LOTRGuiMenuBase {
	...
	private void setupMapWidgets() {
		...
		widgetRenameCWP = new LOTRGuiMapWidget(-16, 34, 10, "renameCWP", 0, 10);
	...
	public void drawScreen(int i, int j, float f) {
		...
		if (hasOverlay) {
			...
			if (overlayLines != null) {
				...
				for (String s3 : overlayLines) {
					displayLines.addAll(fontRendererObj.listFormattedStringToWidth(s3, stringWidth));
				}
	...
	private void renderPlayers(int mouseX, int mouseY) {
		...
		int iconWidthHalf = 4;
		HashMap<UUID, PlayerLocationInfo> playersToRender = new HashMap<>();
	...
	private void openOverlayCreate() {
		...
		if (remaining <= 0) {
			...
			overlayLines = new String[] { StatCollector.translateToLocalFormatted("lotr.gui.map.customWaypoint.create.1", numWaypoints, maxWaypoints), "", StatCollector.translateToLocalFormatted("lotr.gui.map.customWaypoint.create.2", new Object[0]) };
