...
public class LOTRKeyHandler {
  ...
  public void KeyInputEvent(InputEvent.KeyInputEvent event) {
    ...
    if (keyBindingMenu.getIsKeyPressed() && mc.currentScreen == null)
      mc.thePlayer.openGui(LOTRMod.instance, 11, (World)mc.theWorld, 0, 0, 0); 
    LOTRPlayerData pd = LOTRLevelData.getData((EntityPlayer)mc.thePlayer);
    boolean usedAlignmentKeys = false;
    Map<LOTRDimension.DimensionRegion, LOTRFaction> lastViewedRegions = new HashMap<LOTRDimension.DimensionRegion, LOTRFaction>();
    LOTRDimension currentDimension = LOTRDimension.getCurrentDimensionWithFallback((World)mc.theWorld);
