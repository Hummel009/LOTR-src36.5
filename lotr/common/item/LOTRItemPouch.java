        ...
        for (int i = 0; i < inv.mainInventory.length; i++) {
          ...
          if (itemstack != null)
            if (itemstack.getItem() instanceof LOTRItemPouch) {
              pouchSlots.add(Integer.valueOf(i));
            } else {
              itemSlots.add(Integer.valueOf(i));
            }  
        } 
        boolean movedAny = false;
        for (Iterator<Integer> iterator = itemSlots.iterator(); iterator.hasNext(); ) {
          int j = ((Integer)iterator.next()).intValue();
          ItemStack itemstack = inv.getStackInSlot(j);
          for (Iterator<Integer> iterator1 = pouchSlots.iterator(); iterator1.hasNext(); ) {
            int p = ((Integer)iterator1.next()).intValue();
