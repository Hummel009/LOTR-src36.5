...
public class LOTRFoods {
    ...
    public LOTRFoods setDrinkVessels(LOTRItemMug.Vessel... vessels) {
        ...
        List<LOTRItemMug.Vessel> placeable = new ArrayList<LOTRItemMug.Vessel>();
        for (LOTRItemMug.Vessel v : this.drinkVessels) {
          if (v.canPlace)
            placeable.add(v); 
        } 
        if (!placeable.isEmpty()) {
          this.drinkVesselsPlaceable = placeable.<LOTRItemMug.Vessel>toArray(new LOTRItemMug.Vessel[0]);
        } else {
